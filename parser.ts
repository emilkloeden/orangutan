...
export default class Parser {
    ...
    parseLetStatement = (): ast.LetStatement | null => {
        ...
        stmt.name = new ast.Identifier(this.currentToken, this.currentToken.literal)
    ...
    parseBlockStatement = (): ast.BlockStatement => {
        ...
        while (!this.currentTokenIs(TokenType.RBRACE) && !this.currentTokenIs(
            TokenType.EOF)
        ) {
            const stmt = this.parseStatement()
    ...
    peekError = (tokenType: TokenType): void => {
        const errorMessage = `Expected next token to be ${tokenType}, got ${this.peekToken.tokenType} instead.`
